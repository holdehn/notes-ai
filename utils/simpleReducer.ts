...
type KeyEventDispatcher = (
  event: KeyboardEvent<Element>,
  dispatch: any,
  state?: any,
) => void;

export const keyEventDispatcherCreator =
  (keyMap: KeyActionMap): KeyEventDispatcher =>
  (event: KeyboardEvent, dispatch: any) => {
...
export const combineFilterArr = (filters: Filter[]) => (x: any) =>
  filters.reduce((b, f) => b || f(x), false);

export type AnyPayloadCaseReducer = CaseReducer<any, PayloadAction<any>>;
export const simpleReducer =
  (field: string): AnyPayloadCaseReducer =>
